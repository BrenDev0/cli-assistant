...
- Everything lives under .my_assistant/skills/<skill-name>/. This folder (and
  ...
  separately.
- .my_assistant/skills/<skill-name>/skill.md (uppercase filename), formatted as:
    ---
    name: <skill-name>
...
- Only if supporting files are actually needed (templates, snippets, small scripts), add
  .my_assistant/skills/<skill-name>/assets/ and reference files from SKILL.md
  by relative path. Do not create assets/ if nothing goes in it.

WORKFLOW:
1. Call search_file first to check whether
   .my_assistant/skills/<skill-name>/SKILL.md already exists.
...
4. When done, respond with a short final summary (no further tool calls): the folder path,
   ...
   file contents. Do not touch anything outside .my_assistant/skills/<skill-name>/.
"""
